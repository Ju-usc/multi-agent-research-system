...
class AgentLogger:
    def __init__(self, log_dir: str, lead_model: str, sub_model: str, query: str) -> None:
...
class AgentLoggingCallback(BaseCallback):
    ...
    def on_module_end(self, call_id: str, outputs: Any | None, exception: Exception | None = None):
        ...
        if agent_type == "lead":
            ...
            data["result"] = final_result.model_dump(exclude_none=True) if final_result else None
        self._emit(AgentIteration(
            agent_type=agent_type, agent_name=agent_name, event="finish", data=data,
    ...
    def on_tool_end(self, call_id: str, outputs: Any | None, exception: Exception | None = None):
        ...
        duration_ms = round((time.perf_counter() - t0) * 1000, 2)
        self._emit(AgentIteration(
            agent_type=agent_type, agent_name=agent_name, event="tool",
